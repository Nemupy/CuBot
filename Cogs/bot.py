...
class AppCmdBot(commands.Cog):
    ...
    async def help(self,ctx,command=None):
        ...
        if command == "help":
            ...
            embed = discord.Embed(title="ヘルプ", description="こんにちはー！\n日本生まれ日本育ちの純国産BOT！Cuです！", colour=0x3498DB)
            embed.add_field(name=":dividers:》目次", value="１．`コマンド一覧`\n２．`BOTの招待方法`\n３．`よくある質問`")
            embed.set_image(url="https://media.discordapp.net/attachments/826812760435195904/913384084003258418/4.gif")
            embed1 = discord.Embed(title="ヘルプ-コマンドリスト",
                                   description="使用可能なコマンド一覧です♪\n各コマンドの詳細は`Cu!help [コマンド名]`で確認できます♪", colour=0x3498DB)
            embed1.add_field(name=":robot: 》BOT", value="`help` `list` `prof` `ping`", inline=False)
            embed1.add_field(
                name=":tools: 》ツール",
            ...
            embed1.add_field(name=":game_die: 》アクティビティ", value="`youtube` `betrayal` `fishington` `poker` `chess`", inline=False)
    ...
    async def list(self,ctx, type=None):
        ...
        embed.add_field(name=":game_die: 》アクティビティ", value="`youtube` `betrayal` `fishington` `poker` `chess`", inline=False)
        ...
        embed5.add_field(
            ...
            value="`youtube`：YouTubeTogetherを開始します。\n"
                  ...
                  "`poker`：PokerNightを開始します。\n"
                  "`chess`：ChessInTheParkを開始します。",
        )
