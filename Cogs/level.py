...
class AppCmdLevel(commands.Cog):
    ...
    async def on_message(self,message):
        ...
        data = cur.fetchone()
        if data is None:
            cur.execute("INSERT INTO level VALUES(?, ?, ?)", (message.author.id, 1, 0))
            conn.commit()
            return
        cur.execute("UPDATE level set exp=? WHERE userid=?", (data[2] + 1, message.author.id))
        conn.commit()
        cur.execute("SELECT * FROM level WHERE userid=?", (message.author.id,))
        data = cur.fetchone()
        if data[2] >= data[1] * 5:
            cur.execute("UPDATE level set level=?,exp=? WHERE userid=?", (data[1] + 1, 0, message.author.id))
            conn.commit()
